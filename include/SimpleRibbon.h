...
class SimpleRibbon {
        #define STATIC_LIBRARY_MESSAGE "This is a static library. You can only select the template arguments that are compiled into the library."
        ...
        static_assert((bits <= 8 && std::is_same<result_t, uint8_t>::value)
                        || (bits == 16 && coeff_bits == 64 && std::is_same<result_t, uint16_t>::value)
